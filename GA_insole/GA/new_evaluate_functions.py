...
def buffer(t, p, Es=20, ps=100, n=2, k1=3, k2=2):
    E = Es * (p / ps) ** n
    return k1 * t - k2 * E ** 2


def support(l, w, t, h, p, Es=20, ps=100, n=2, v=0.07, k3a=1, k3b=1, k3c=1, k3d=1):
    rs = l ** 2 / (8 * h)
    E = Es * (p / ps) ** n
    return k3a * h - k3b * w / l - k3c * t ** 5 / rs + k3d * E * t ** 3 / (12 * (1 - v ** 2))


def weight(l, w, t, h, p):
    return l * w * (t + h / 3) * p
...
def total(l, w, t, h, p):
    return w1 * buffer(t, p) + w2 * support(l, w, t, h, p) + w3 * weight(l, w, t, h, p) + w4 * fit(l, w, h)
